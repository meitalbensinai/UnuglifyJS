...
function replaceMangled(code, file, infered_names, max_path_length, original_features) {
	...
	if (original_features) {
		...
		generateGenericFeatures(toplevel, feature_outputter, max_path_length);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function extractFeatures(code, file, print_ast, max_path_length, skip_minified, original_features, features) {
	...
	return feature_outputter.output.join("");
}
...
function generatePathFeature(sourceStack, targetStack, max_path_length) {
	...
	pathParts.push('(' + commonNodeString + ')');
	...
	var feature = pathParts.join("");
	return hashCode(feature);
}

function generateGenericFeatures(toplevel, feature_outputter, max_path_length) {
	var nodes = {};
	var leaves = [];
	var walker = new TreeWalker(function(node){
		var stack = walker.stack;
		//feature_outputter.string_map.addKey(node.id);
		nodes[node.id] = {node: node, stack: stack.slice()};
	...
	for (var sourceIndex = 0, leavesLength = leaves.length ; sourceIndex < leavesLength ; sourceIndex++) {
		for (var targetIndex = 0 ; targetIndex < leavesLength ; targetIndex++) {
			...
			var feature = generatePathFeature(nodes[sourceId].stack, nodes[targetId].stack, max_path_length);
			// nodeToProperty hides a hidden language-specific bias - INFER/GIVEN, and types that should be inferred
			if (feature != undefined) {
				var sourceProperty = feature_outputter.addPathFeature(nodeToProperty(nodes[sourceId].node), nodeToProperty(nodes[targetId].node), feature);
			}
		}
	}
}
