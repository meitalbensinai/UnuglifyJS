...
function replaceMangled(code, file, infered_names, max_path_length, max_path_width, original_features, hashCode) {
	var toplevel;
	try {
	...
	if (original_features) {
		...
		generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hashCode);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function extractFeatures(code, file, print_ast, max_path_length, max_path_width, skip_minified, original_features, features, hashCode) {
	var toplevel;
...
function generatePathFeature(sourceStack, targetStack, max_path_length, max_path_width) {
	var commonPrefix = 0;
	...
	if ((pathLength > max_path_length || pathWidth > max_path_width) && (!sameVariableByStacks(sourceStack, targetStack))) {
		return;
	}
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hashCode) {
	...
	for (var sourceIndex = 0 ; sourceIndex < leavesIndex ; sourceIndex++) {
		for (var targetIndex = sourceIndex + 1 ; targetIndex < leavesIndex ; targetIndex++) {
			...
			if (!sourcePropery.must_infer && !targetProperty.must_infer) {
				continue;
			}

			var featureParts = generatePathFeature(sourceStack, targetStack, max_path_length, max_path_width);
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hashCode) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hashCode);
		}
	}));
}
