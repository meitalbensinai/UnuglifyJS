...
function extractFeatures(code, file, print_ast, max_path_length, max_path_width, skip_minified, original_features, features, hasher, skip_unary_features) {
	var toplevel;
	...
	if (original_features) {
		...
		generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features) {
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features);
		}
	}));
}
