...
function replaceMangled(code, file, infered_names, max_path_length, original_features) {
	var toplevel;
	try {
	...
	var counters = extendAst(toplevel);
	...
	if (original_features) {
		...
		generateGenericFeatures(toplevel, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function extractFeatures(code, file, print_ast, max_path_length, skip_minified, original_features, features) {
	var toplevel;
...
function generatePathFeature(sourceStack, targetStack, max_path_length) {
	var commonPrefix = 0;
	...
	for (var i = sourceStack.length - 1 ; i >= commonPrefix ; i-- ) {
		...
		pathParts[pathIndex] = ('(' + nodeString + ((i == sourceStack.length - 1)? node.child_id : '') + ')');
		pathIndex++;
		pathParts[pathIndex] = '^';
		pathIndex++;
	}
	...
	for (var i = commonPrefix, targetStackLength = targetStack.length ; i<targetStackLength ; i++) {
		...
		pathIndex++;
		pathParts[pathIndex] = ('(' + nodeString + ((i == targetStackLength -1 ) ? node.child_id : '') + ')');
		pathIndex++;
	}
	return pathParts;
}

function generateGenericFeatures(toplevel, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves) {
	...
	for (var sourceIndex = 0, leavesLength = leaves.length ; sourceIndex < leavesLength ; sourceIndex++) {
		for (var targetIndex = sourceIndex + 1 ; targetIndex < leavesLength ; targetIndex++) {
			...
			var featureParts = generatePathFeature(sourceStack, targetStack, max_path_length);
			...
			if (featureParts != undefined) {
				var feature = hashCode(featureParts.join(""));
...
function generateFnamesFeatures(toplevel, feature_outputter){
	...
	toplevel.walk(new TreeWalker(function(node, descend){
		...
		} else if (node instanceof AST_Return && node.value.toProperty != null) {
			addFeatures(outer_funcs, "FN", node.value.toProperty, "RETURN", feature_outputter);
		}
	}));
}
...
FeatureJsonOutputter.prototype.addFeature = function(a, b, name){
	...
	this.output[this.outputIndex++]='"a": ' + a_id + ",";
	this.output[this.outputIndex++]='\t"b": ' + b_id + ",";
	this.output[this.outputIndex++]='\t"f2": "' + name + '"';
...
FeatureJsonOutputter.prototype.addPathFeature = function(source, target, featureString){
	...
	if (!source.must_infer && !target.must_infer) {
		return;
	}
	...
	this.output[this.outputIndex++]='"a": ' + a_id + ",";
	this.output[this.outputIndex++]='\t"b": ' + b_id + ",";
	this.output[this.outputIndex++]='\t"f2": "' + featureString + '"';
...
FeatureJsonOutputter.prototype.addSymbol = function(key){
	...
	this.output[this.outputIndex++]='"v": ' + this.string_map.getId(key) + ",";
	if (key[0] == INFER){
		//${id}-{name}		
		this.output[this.outputIndex++]='\t"inf": "' + escapeString(key.split("-")[1]) + '"';
	} else {
		//#{name}
		this.output[this.outputIndex++]='\t"giv": "' + escapeString(key.slice(1)) + '"';
	}
...
FeatureJsonOutputter.prototype.endScope = function(){
	...
	this.openElem();
	this.output[this.outputIndex++]='"cn":"!=", "n":[';

	this.output[this.outputIndex++]=keys[0];
	for(var i = 1,length = keys.length; i < length; i++ ) {
		this.output[this.outputIndex++]=',';
		this.output[this.outputIndex++]=keys[i];
	}
