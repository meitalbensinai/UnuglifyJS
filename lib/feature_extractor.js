...
function extractFeatures(code, file, print_ast, max_path_length, max_path_width, skip_minified, original_features, features, hasher) {
	var toplevel;
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher) {
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher);
		}
	}));
}
