...
function replaceMangled(code, file, infered_names, max_path_length, original_features) {
	...
	if (original_features) {
		...
		generateAutoFeatures(toplevel, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function generatePathFeature(sourceStack, targetStack, max_path_length) {
	...
	if (pathLength > max_path_length && (!sameVariableByStacks(sourceStack, targetStack))) {
		return;
	}
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves) {
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, numberOfNodes, numberOfLeaves);
		}
	}));
}
