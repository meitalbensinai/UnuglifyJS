...
function replaceMangled(code, file, infered_names, max_path_length, max_path_width, original_features) {
	var toplevel;
	try {
	...
	if (original_features) {
		...
		generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
var hashCode;
function extractFeatures(code, file, print_ast, max_path_length, max_path_width, skip_minified, original_features, features, hashFunction) {
	var toplevel;
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves) {
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves);
		}
	}));
}
