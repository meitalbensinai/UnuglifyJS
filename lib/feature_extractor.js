...
function extractFeatures(code, file, print_ast, max_path_length, max_path_width, skip_minified,
						 original_features, features, hasher, skip_unary_features, include_giv_giv) {
	var toplevel;
	...
	if (original_features) {
		...
		generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features, include_giv_giv);
		generateFscopeConstraints(toplevel, feature_outputter);
	}
...
function generateAutoFeaturesForSubtree(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features, include_giv_giv) {
...
function generateAutoFeatures(toplevel, feature_outputter, max_path_length, max_path_width, numberOfNodes,
							  numberOfLeaves, hasher, skip_unary_features, include_giv_giv) {
	toplevel.walk(new TreeWalker(function(node) {
		if (node instanceof AST_Defun || node instanceof AST_Lambda) {
			generateAutoFeaturesForSubtree(node, feature_outputter, max_path_length, max_path_width, numberOfNodes, numberOfLeaves, hasher, skip_unary_features, include_giv_giv);
		}
	}));
}
